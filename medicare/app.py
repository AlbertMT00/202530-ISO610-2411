...
def crear_cita():
    ...
    try:
        ...
        # Validar que no se agenden citas en horas que ya pasaron completamente (si es hoy)
        ...
        if fecha_obj == fecha_hoy:
            ...
            hora_seleccionada = int(hora.split(':')[0])
            
            # Solo bloquear si la hora YA PASÓ COMPLETAMENTE
            # Ejemplo: Si son las 15:30, se pueden agendar citas a las 15:00
            # Solo bloquear horas anteriores a la actual
            if hora_seleccionada < hora_actual:
                flash(f'No se puede agendar en horas que ya pasaron. Hora actual: {hora_actual}:{datetime.now().minute:02d}', 'error')
                return redirect(url_for('citas'))
...
def horarios_disponibles():
    ...
    for h in horarios_ocupados_raw:
        ...
        # Agregar también la hora siguiente (si existe una cita, ocupa 1 hora completa)
        # Esto previene que alguien reserve en medio de una cita existente
        try:
            hora_obj = datetime.strptime(hora_str, '%H:%M')
            hora_siguiente = (hora_obj + timedelta(minutes=30)).strftime('%H:%M')
            ocupados.append(hora_siguiente)
            hora_siguiente2 = (hora_obj + timedelta(hours=1)).strftime('%H:%M')
            ocupados.append(hora_siguiente2)
        except:
            pass
